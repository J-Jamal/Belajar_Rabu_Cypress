...
//   cy.get("#email_address").type("cobaa99@gmail.com");
...
//     .click()
//     .should("be.visible")
//     .should("contain", "Create an Account"); //cek tulisan create account
//   cy.get(".message-success")
//     .should("be.visible")
//     .should("contain", "Thank you for registering with Main Website Store"); //Exspected result
...
describe("User Masuk Ke Halaman Create Account", () => {
  ...
  //   cy.get("#password").type("123AB");
  //   cy.get("#password-confirmation").type("123AB");
  //   cy.get(
  //     "#form-validate > .actions-toolbar > div.primary > .action > span"
  //   ).click();
  //   cy.get("#password-error")
  ...
  it("TC-06 User create account memasukan password dengan angka", function () {
    ...
    cy.get("#password").type("StroPass123!");
    cy.get("#password-confirmation").type("StrongPassword123!");

    // cy.get(
    //   "#form-validate > .actions-toolbar > div.primary > .action > span"
    // ).click();

    // Verify that the password strength meter is visible
    cy.get("#password-strength-meter")
      .should("be.visible")
      .should("contain", "Password Strength:");

    // Verify that the password strength meter label contains "Very Strong", "Strong", "Medium" or "Weak"
    cy.get("#password-strength-meter-label")
      .invoke("text")
      .then((text) => {
        expect(text).to.match(/Very Strong|Strong|Medium|Weak/);
      });
  });
